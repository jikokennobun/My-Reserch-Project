...
if ([string]::IsNullOrWhiteSpace($ChannelName)) {
    $ChannelName = -join @([char]0x8996, [char]0x8074, [char]0x30ED, [char]0x30B0)
}
if ([string]::IsNullOrWhiteSpace($Topic)) {
    $Topic = -join @(
        [char]0x898B, [char]0x305F, [char]0x30A2, [char]0x30CB, [char]0x30E1,
        [char]0x30FB, [char]0x52D5, [char]0x753B, [char]0x30FB, [char]0x914D,
        [char]0x4FE1, [char]0x306E, [char]0x30ED, [char]0x30B0, [char]0x3002,
        [char]0x0055, [char]0x0052, [char]0x004C, [char]0x3068, [char]0x77ED, [char]0x3044, [char]0x30E1,
        [char]0x30E2, [char]0x3092, [char]0x6295, [char]0x3052, [char]0x308B,
        [char]0x3002
    )
}
...
function Invoke-DiscordJson {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "curl.exe failed for $methodName $Path."
    }
...
if ($DryRun) {
    Write-Host "Would ensure Discord watch channel '$ChannelName' in guild $GuildId."
    if ($StoreInUserEnvironment) {
        Write-Host "Would update DISCORD_WATCH_CHANNEL_ID and DISCORD_WATCH_CHANNEL_NAME."
    }
    exit 0
}

$channels = @(Invoke-DiscordJson -Method Get -Path "/guilds/$GuildId/channels")
$storedChannelId = [Environment]::GetEnvironmentVariable("DISCORD_WATCH_CHANNEL_ID", "User")
$channel = $null
...
if ($null -eq $channel) {
    ...
    Write-Host "Created Discord watch channel '$($channel.name)' with id $($channel.id)."
} else {
    Write-Host "Found Discord watch channel '$($channel.name)' with id $($channel.id)."
}

if ($StoreInUserEnvironment) {
    [Environment]::SetEnvironmentVariable("DISCORD_WATCH_CHANNEL_ID", [string]$channel.id, "User")
    [Environment]::SetEnvironmentVariable("DISCORD_WATCH_CHANNEL_NAME", [string]$channel.name, "User")
    Write-Host "Updated Windows user environment for watch channel."
}

Write-Host "Watch channel: $($channel.name) ($($channel.id))"
