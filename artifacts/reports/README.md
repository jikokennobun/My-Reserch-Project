...
- `pass108-integral-equivariant-repair-section-check.json`: verifies the
  ...
  `(Z^S)^{Sym(S)}` is generated by the all-ones vector, so `Sigma_S` maps it
  to `|S|Z`; that the rational barycenter gives a symmetric section over `Q`
  with exact denominator `|S|`; that an equivariant integral lift of `m`
  exists exactly when `|S|` divides `m`; that barycentric sections are not
  natural under support inclusions but differ by rational kernel elements; and
  that the antipode sign does not change the denominator obstruction.
