...
- **(Pass 100 -> 101: oriented-support groupoid and antipode quotient)**
  Pass 100 defined the primitive orientation torsor
  $$\mathcal O_S=\{c\in\mathbb Z^S:\sum c_p=0,\gcd(c_p)=1\}.$$
  For $S\subseteq T$, orientations pull back by zero-extension
  $\mathcal O_S\to\mathcal O_T$, preserving primitivity, collapse
  compatibility, finite kernel factorization, and the antipode
  $c\mapsto-c$.  The reverse direction is not canonical: restriction can fail
  the zero-sum condition.  No support-symmetric primitive orientation exists,
  so the all-prime generator is obtained only after choosing, quotienting, or
  forgetting the orientation torsor.  Open:
  (i) define the oriented-support groupoid/stack with objects $(S,c)$ and
  morphisms generated by zero-extension, support refinements, and antipode;
  (ii) compute whether its antipode quotient retains exactly the Pass-94
  functional-equation sign or loses too much orientation data;
  (iii) decide how to present the all-prime constant-term generator as a
  quotient/colimit of this oriented groupoid while preserving the finite
  torsion shadow bookkeeping.
