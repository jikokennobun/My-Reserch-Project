...
- **(Pass 108 -> 109: barycentric transition denominators under support inclusions)**
  Pass 108 classified support-symmetric repair sections.  For $|S|=n$, the
  invariant lattice in $\mathbb Z^S$ is generated by $\mathbf 1_S$, whose
  summation is $n$; hence invariant integral vectors map to $n\mathbb Z$ and
  no support-symmetric integral section of $\Sigma_S$ exists for $n>1$.
  Over $\mathbb Q$, the barycentric section
  $s_{\mathrm{bar}}(1)=\frac1n\mathbf 1_S$ is support-symmetric, so the exact
  obstruction is the denominator class $\mathbb Z/n\mathbb Z$.  The antipode
  local system is independent of this denominator.  Open:
  (i) compute the rational transition
  $e_{S,T}s_{\mathrm{bar},S}-s_{\mathrm{bar},T}$ under support inclusions
  $S\subseteq T$ and its denominator;
  (ii) decide whether these denominators match finite conductor/CRT
  denominator bookkeeping in the signed boundary package;
  (iii) determine whether the barycentric transition class can be useful as
  a normalized rational comparison even though it has no integral
  support-symmetric lift.
